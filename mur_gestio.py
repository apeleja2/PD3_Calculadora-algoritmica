...
try:
    ...
    if df.empty:
        ...
        text_cerca = st.sidebar.text_input("Cerca per paraula clau")
        ...
        paraules_buides = {
            ...
            "més", "molt", "nan", "si", "no", "del", "dels", "de", "amb", "sobre", "tot", "tota", 
            "tots", "totes", "hi", "ha", "han", "va", "van", "però", "o", "com", "ens", "li", "als", "altre", 
            "altres", "ser", "fer", "fet", "estat", "està", "bastant", "només", "encara", "això"
        }
        ...
        if len(text_total.strip()) > 10:
            ...
            st.info("No hi ha prou text per generar l'anàlisi.")
        ...
        if df_filtrat.empty:
            st.info("No s'ha trobat cap resposta.")
        else:
            ...
            with tab_cards:
                ...
                for i, (_, row) in enumerate(df_filtrat.iterrows()):
                    if i >= max_mostrades:
                        st.warning(f"S'estan mostrant les primeres {max_mostrades}.")
                        break
